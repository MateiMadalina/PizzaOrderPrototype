...
const displayPizzas = (element, ingredients, allergens) => {
  return `<div id="divPizza" data-id="${element.id}">
              ...
              <div id="ingredient">
                ...
                <ul id="allIngredients">${ingredients.join("")}</ul>
              </div>
              <div id="alergens">
                <h5>Allergens</h5>
                <h6 id="allAlergens">${allergens.join("")}</h6>
              </div>
...
const data = async () => {
  ...
  const pizzaMap = databasePizza.map((element) => {
    ...
    return displayPizzas(element, ingredientsArray, allergensMap);
  });
