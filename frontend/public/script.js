...
const data = async () => {
  ...
  if (allergensArr.length === 0) {
    ...
    while (document.querySelector("#menu").firstChild) {
        document.querySelector("#menu").removeChild(document.querySelector("#menu").firstChild);
    }
    const pizzaMap = databasePizza.map((element) => {
        const allergensMap = databaseAllergens.map((allergen) => {
          const idAllergens = element.allergens.map((id) => {
       ...
       console.log(pizzaMap[0])
       let a = [];
      allergensArr.map(item => {
        pizzaMap.forEach((el,i,arr)=>{
            if(!arr[i].includes(item)){
               a.push(arr[i])
            }
        })
      })
      document.querySelector("#pizzaList").insertAdjacentHTML("beforeend", a.join(""));
  }
